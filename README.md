...
<script>
// ══ SUPABASE CLIENT ════════════════════════════════════════════
const SUPABASE_URL = 'https://obwcntliainlbxuzfeew.supabase.co';
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im9id2NudGxpYWlubGJ4dXpmZWV3Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzMyMDk1MDQsImV4cCI6MjA4ODc4NTUwNH0.JUysYTQX9LMkdO2fiDjkhoOrh8rf4Km6tZB0ePsiYXA';
...
async function getSupaUser() {
  if (_supaUser) return _supaUser;
  const { data } = await _supa.auth.getUser();
  _supaUser = data?.user || null;
  return _supaUser;
}

// ── Load projects from Supabase on login ────────────────────────
async function loadProjectsFromSupabase() {
  ...
  (data || []).forEach(row => {
    PROJECTS_DB.push({
      ...
      client: {
        attention: row.owner_name || '',
        tel: row.owner_phone || '',
        address: row.address || '',
        contractAmt: parseFloat(row.contract_amount) || 0,
      },
...
<div class="portal-bar">
  ...
  <button class="portal-btn active" onclick="switchPortal('designer')" id="btn-designer">
    <span class="portal-dot"></span> <span data-i18n="nav_designer">设计师端</span>
  </button>
  <button class="portal-btn" onclick="switchPortal('owner')" id="btn-owner">
    <span class="portal-dot"></span> <span data-i18n="nav_owner">业主端</span>
  </button>
  <button class="portal-btn" onclick="switchPortal('worker')" id="btn-worker">
    <span class="portal-dot"></span> <span data-i18n="nav_worker">工人端</span>
  </button>
  ...
  <div class="designer-layout">
    ...
    <div class="sidebar">
      <div style="padding: 0 16px 20px; border-bottom: 1px solid var(--border2); margin-bottom: 12px;">
        <div style="font-size:12px; color:var(--text3);" data-i18n="sidebar_logged_as">登录身份</div>
      ...
      <div class="sidebar-label" data-i18n="sidebar_workspace">工作台</div>
      <div class="sidebar-section">
        <div class="nav-item active" onclick="showPanel('upload')">
          <span class="nav-icon">📋</span> <span data-i18n="nav_quotation">报价单导入</span>
        </div>
        <div class="nav-item" onclick="showPanel('review')">
          <span class="nav-icon">🤖</span> <span data-i18n="nav_ai_review">AI 审核结果</span>
          <span class="nav-badge">3</span>
        </div>
        <div class="nav-item" onclick="showPanel('schedule')">
          <span class="nav-icon">📅</span> <span data-i18n="nav_schedule">工程进度编排</span>
        </div>
        <div class="nav-item" onclick="showPanel('export')">
          <span class="nav-icon">📤</span> <span data-i18n="nav_export">导出报告</span>
        </div>
      </div>

      <div class="sidebar-label" data-i18n="sidebar_projects">项目</div>
      ...
      <div class="sidebar-label" data-i18n="sidebar_team">团队</div>
      <div class="sidebar-section">
        <div class="nav-item" onclick="showPanel('workers')">
          <span class="nav-icon">👷</span> <span data-i18n="nav_workers">工人名册</span>
          <span class="nav-badge" style="background:var(--orange)">1</span>
        </div>
      </div>

      <div class="sidebar-label" data-i18n="sidebar_settings">设置</div>
      <div class="sidebar-section">
        ...
        <div class="nav-item" onclick="showPanel('pricelibrary');setTimeout(checkPriceLibAccess,80)">
          <span class="nav-icon">💰</span> <span data-i18n="nav_pricelist">价格数据库</span>
        </div>
    ...
    <div class="main-content">
      ...
      <div class="panel active" id="panel-upload">
        <div class="panel-title" data-i18n="upload_title">上传报价单</div>
        <div class="panel-sub" data-i18n="upload_sub">支持 Excel (.xlsx/.xls)、CSV、PDF · AI 自动读取并审核</div>
        ...
        <div class="upload-zone" id="upload-zone"
          ...
          <div class="upload-text"><strong data-i18n="upload_click">点击上传</strong> <span data-i18n="upload_or">或将文件拖放至此处</span></div>
          <div class="upload-formats" data-i18n="upload_formats">支持 .xlsx · .xls · .csv · .pdf · 最大 10MB</div>
        ...
        <div id="ai-loading-state" style="display:none; text-align:center; padding:40px 20px;">
          ...
          <div style="font-size:16px; font-weight:600; color:var(--gold); margin-bottom:8px;" id="ai-status-text" data-i18n="ai_reading">正在读取文件...</div>
          <div style="font-size:13px; color:var(--text2);" data-i18n="ai_analyzing">AI 正在分析报价单，对比市场价格库...</div>
        ...
        <div id="upload-preview" style="display:none">
          ...
          <div id="file-info-strip" style="display:flex;align-items:center;gap:12px;background:var(--surface);border:1px solid var(--border2);border-radius:10px;padding:12px 16px;margin-bottom:20px;">
            ...
            <button class="btn btn-ghost" style="padding:5px 12px;font-size:12px;" onclick="resetUpload()" data-i18n="btn_reupload">重新上传</button>
          </div>
          ...
          <div class="ai-review-card" id="ai-score-card">
            <div class="ai-header">
              ...
              <div class="ai-title" id="ai-score-title" data-i18n="ai_score_title">综合评分 & 风险评估</div>
              <div style="margin-left:auto;font-size:12px;color:var(--text2);" data-i18n="ai_market_data">基于 MY/SG 2025 市场数据</div>
            </div>
          ...
          <div style="margin:20px 0 0; display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:8px;">
            <div class="section-title" data-i18n="tbl_items_title">已识别工程项目</div>
            ...
            <div style="overflow-x:auto;">
              <table class="q-table" style="width:100%" id="q-table-body">
                <thead>
                  <tr>
                    <th>#</th>
                    <th data-i18n="tbl_desc">工程项目</th>
                    <th data-i18n="tbl_unit">单位</th>
                    <th data-i18n="tbl_qty">数量</th>
                    <th data-i18n="tbl_unitprice">单价</th>
                    <th data-i18n="tbl_total">小计</th>
                    <th data-i18n="tbl_status">AI 状态</th>
                  </tr>
                </thead>
...
<div class="modal-overlay" id="modal-register">
  <div class="modal-box">
    ...
    <div id="reg-step-1">
      ...
      <div class="modal-sub">输入手机号码即可登入 · 无需验证码</div>
      <div class="form-group">
    ...
    <div id="reg-step-2" style="display:none">
      ...
      <div class="modal-sub" id="otp-hint">输入手机号码即可登入</div>
      <div class="otp-row">
...
function resetUpload() {
  ...
  window.lastExtractedClient = null;
  window.projectClientData   = null;
  window.lastDiff            = null;
  window.previousQuotationItems = null;
  ['ce-company','ce-attention','ce-tel','ce-email','ce-address','ce-ref'].forEach(id => {
...
必查漏项：防水工程(湿区)、水电暗槽布线、清洁及废料处理。`;
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      console.error('AI API Error:', resp.status, errData);
      if (resp.status === 401) {
        showToast('❌', 'API Key 无效，请检查 Vercel 环境变量设置');
        setAIProgress(100, '❌ API 认证失败');
        return;
      }
      throw new Error(`API ${resp.status}: ${errData.error?.message || 'Unknown error'}`);
    }
    ...
    if (data.error) {
      ...
      showToast('⚠️', `AI 错误: ${data.error.message || '请重试'}`);
      setAIProgress(100, '❌ AI 分析失败，使用演示数据');
    ...
    console.error('Quotation analysis error:', err);
    showToast('⚠️', `AI 分析失败（${err.message}）— 显示演示数据`);
...
// ── Send OTP (simulated) ──────────────────────────────────────────
async function authSendOTP(portal) {
  // Phone-only mode: OTP disabled, call authLoginWithPhone directly
  authLoginWithPhone(portal);
}
...
async function authVerifyOTP(portal) {
  const p = portal === 'designer' ? 'd' : portal === 'owner' ? 'o' : 'w';
  const phoneInput = document.getElementById('auth-' + p + '-phone');
  const rawPhone = (phoneInput?.value || '').trim().replace(/[\s\-()]/g,'');
  const phone = rawPhone.startsWith('+') ? rawPhone : '+60' + rawPhone.replace(/^0/,'');
  const token = [...document.querySelectorAll('#auth-' + portal + ' .auth-otp-box')]
    .map(b => b.value).join('');

  if (token.length < 6) { showToast('⚠️','请输入6位验证码'); return; }

  // Demo shortcut
  if (token === '888888') { authDemoLogin(portal); return; }

  const { data, error } = await _supa.auth.verifyOtp({ phone, token, type: 'sms' });
  if (error) {
    showToast('❌','验证码错误，请重试');
    return;
  }
  ...
  await _supa.from('profiles').upsert({ id: data.user.id, role: portal, phone }, { onConflict: 'id' });
  authDemoLogin(portal);
  if (portal === 'designer') loadProjectsFromSupabase();
}
...
async function authLoginWithPhone(portal) {
  const p = portal === 'designer' ? 'd' : portal === 'owner' ? 'o' : 'w';
  const phoneInput = document.getElementById('auth-' + p + '-phone');
  const rawPhone = (phoneInput?.value || '').trim().replace(/[\s\-()]/g,'');

  if (!rawPhone) {
    // No phone entered — demo mode
    authDemoLogin(portal);
    return;
  }

  const phone = '+60' + rawPhone.replace(/^(\+60|0)/,'');

  // Save phone to local state
  if (portal === 'designer') AUTH_STATE.currentUser.designer.phone = phone;
  if (portal === 'owner')    AUTH_STATE.currentUser.owner.phone    = phone;
  if (portal === 'worker')   AUTH_STATE.currentUser.worker.phone   = phone;

  showToast('✅', `欢迎！${phone}`);

  // Save to Supabase anonymously (phone as identifier)
  _supa.from('profiles').upsert({
    id: btoa(phone).replace(/[^a-z0-9]/gi,'').substring(0,36).padEnd(36,'0')
      .replace(/(.{8})(.{4})(.{4})(.{4})(.{12})/, '$1-$2-$3-$4-$5'),
    role: portal,
    phone: phone,
    name: AUTH_STATE.currentUser[portal]?.name || '',
...
function workerRegisterSendOTP() {
  // Phone-only mode: skip OTP, register directly
  workerRegisterComplete();
  return;
  // Original OTP flow below (disabled):
...
document.addEventListener('DOMContentLoaded', () => {
  // Apply Chinese language on load
  if (typeof applyI18n === 'function') applyI18n('zh');
...
function confirmSaveProject() {
  ...
  // 同步保存到 Supabase
  getSupaUser().then(user => {
    if (!user) return;
    _supa.from('projects').insert({
      id: proj.id,
      designer_id: user.id,
      project_name: name,
      owner_name: client.attention,
      owner_phone: client.tel,
      address: client.address,
      contract_amount: client.contractAmt || null,
      schedule_items: proj.scheduleItems,
      task_assignments: proj.taskAssignments,
      status: 'draft',
      published: false,
    }).then(({ error }) => {
      if (error) console.warn('Save project error:', error.message);
      else showToast('☁️','已同步到云端');
    });
  });
}
...
<div class="auth-overlay" id="auth-designer">
  <div class="auth-card">
    ...
    <div class="auth-step" id="auth-d-login-otp">
      ...
      <div style="font-size:10px;color:var(--text3);margin-top:8px;">（电话登入已启用，无需验证码） <button class="auth-send-btn" onclick="authAutoFillOTP('designer-l')">自动填入</button></div>
    ...
    <div class="auth-step" id="auth-d-register">
      ...
      <button class="auth-btn auth-btn-blue" onclick="authDemoLogin('designer')">立即注册 · 免费体验 →</button>
...
<div class="auth-overlay hidden" id="auth-owner">
  <div class="auth-card">
    ...
    <div class="auth-step" id="auth-o-login-otp">
      ...
      <div style="font-size:10px;color:var(--text3);margin-top:8px;">（电话登入已启用，无需验证码） <button class="auth-send-btn" onclick="authAutoFillOTP('owner-l')">自动填入</button></div>
...
[index.html](https://github.com/user-attachments/files/25902160/index.html)
!DOCTYPE html>
<html lang="zh">
<head>
<meta charset="UTF-8">
