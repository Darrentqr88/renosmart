...
export default function WorkerProfileTab({ profile, sessionUserId, tasks }: WorkerProfileTabProps) {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Header */}
      <div className="bg-[#0F1923] text-white px-5 pt-12 pb-8">
        {/* Avatar */}
        <div className="flex items-center gap-4">
          <div className="w-16 h-16 rounded-2xl bg-[#F0B90B] flex items-center justify-center flex-shrink-0">
        ...
        <div className="grid grid-cols-2 gap-3 mt-5">
          <div className="bg-white/8 rounded-xl px-3 py-2.5" style={{ background: 'rgba(255,255,255,0.06)' }}>
            ...
            <p className="text-2xl font-bold text-[#F0B90B] mt-0.5">{completedTasks}</p>
          </div>
          <div className="bg-white/8 rounded-xl px-3 py-2.5" style={{ background: 'rgba(255,255,255,0.06)' }}>
            <p className="text-[10px] text-white/40 font-medium">Receipts Submitted</p>
            <p className="text-2xl font-bold text-[#F0B90B] mt-0.5">{receiptCount}</p>
          </div>
        </div>
      </div>

      {/* Content */}
      <div className="flex-1 overflow-y-auto bg-[#F7F8FA] p-4 space-y-4">

        {/* Personal info (read-only) */}
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
          <div className="px-4 py-3 border-b border-gray-50">
            <p className="text-[11px] font-bold text-gray-400 uppercase tracking-wide">Personal</p>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-gray-50">
            <p className="text-[11px] font-bold text-gray-400 uppercase tracking-wide">Company Info</p>
            {editing && (
              <div className="flex gap-2">
                <button onClick={() => setEditing(false)} className="p-1 rounded-lg hover:bg-gray-100">
                  <X className="w-3.5 h-3.5 text-gray-400" />
                </button>
                <button onClick={handleSave} disabled={saving} className="flex items-center gap-1 px-2.5 py-1 bg-[#F0B90B] text-black rounded-lg text-[11px] font-semibold">
          ...
          <div className="divide-y divide-gray-50">
            {editing ? (
              <div className="p-4 space-y-3">
                <div>
                  <label className="text-[11px] text-gray-500 font-medium block mb-1">Company Name</label>
                  <input value={company} onChange={e => setCompany(e.target.value)} placeholder="Your company name" className="w-full text-sm border border-gray-200 rounded-xl px-3 py-2.5 focus:outline-none focus:border-[#F0B90B]" />
                </div>
                <div>
                  <label className="text-[11px] text-gray-500 font-medium block mb-1">Company Address</label>
                  <input value={companyAddress} onChange={e => setCompanyAddress(e.target.value)} placeholder="Company address" className="w-full text-sm border border-gray-200 rounded-xl px-3 py-2.5 focus:outline-none focus:border-[#F0B90B]" />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-[11px] text-gray-500 font-medium block mb-1">SSM No.</label>
                    <input value={ssmNo} onChange={e => setSsmNo(e.target.value)} placeholder="e.g. 202301234567" className="w-full text-sm border border-gray-200 rounded-xl px-3 py-2.5 focus:outline-none focus:border-[#F0B90B]" />
                  </div>
                  <div>
                    <label className="text-[11px] text-gray-500 font-medium block mb-1">Employees</label>
                    <input type="number" value={employeeCount} onChange={e => setEmployeeCount(e.target.value)} placeholder="e.g. 5" className="w-full text-sm border border-gray-200 rounded-xl px-3 py-2.5 focus:outline-none focus:border-[#F0B90B]" />
                  </div>
                </div>
                <div>
                  <label className="text-[11px] text-gray-500 font-medium block mb-1">Project Value Range (RM)</label>
                  <div className="grid grid-cols-2 gap-3">
                    <input type="number" value={minValue} onChange={e => setMinValue(e.target.value)} placeholder="Min e.g. 5000" className="w-full text-sm border border-gray-200 rounded-xl px-3 py-2.5 focus:outline-none focus:border-[#F0B90B]" />
                    <input type="number" value={maxValue} onChange={e => setMaxValue(e.target.value)} placeholder="Max e.g. 500000" className="w-full text-sm border border-gray-200 rounded-xl px-3 py-2.5 focus:outline-none focus:border-[#F0B90B]" />
                  </div>
                </div>
                <div>
                  <label className="text-[11px] text-gray-500 font-medium block mb-2">Trades (select all that apply)</label>
                  <div className="flex flex-wrap gap-2">
                    {WORKER_TRADES.map(trade => (
                      <button
                        ...
                        style={selectedTrades.includes(trade) ? { background: `${TRADE_COLORS[trade] || '#F0B90B'}25`, color: TRADE_COLORS[trade] || '#F0B90B', border: `1px solid ${TRADE_COLORS[trade] || '#F0B90B'}50` } : {}}
                      >
                        {trade}
                      </button>
                    ))}
